...
<head>
    ...
    <title>Laporan Audit Financial - Kelompok 04</title>
    ...
    <div class="row no-gutters">
        ...
        <div class="col-12 col-md-3 col-xl-2 cyber-sidebar">
            ...
            <div class="px-2">
                ...
                <a href="{{ route('keuangan.laporan.index') }}" class="nav-link-cyber active">
            ...
            <div class="mb-4">
                <div style="border-left: 4px solid #00ffcc; padding-left: 15px; background: linear-gradient(90deg, #141419 0%, transparent 100%); padding-top: 10px; padding-bottom: 10px; border-radius: 0 8px 8px 0;">
                    <h1 class="m-0" style="color: #00ffcc; text-shadow: 0 0 12px rgba(0,255,204,0.4); font-weight: 800; letter-spacing: 1px; font-size: 26px;">
                        LAPORAN AUDIT HISTORI
                    </h1>
                    <p class="text-muted mb-0" style="font-size: 13px;">Rekam Jejak Transaksi Masuk dan Keluar Dana • UNIBA Madura</p>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-dark text-nowrap m-0">
                        ...
                        <tbody style="font-size: 14px;">
                            @forelse($semuaLaporan as $l)
                            <tr style="border-bottom: 1px solid #2d3748;">
                                <td class="align-middle font-weight-bold" style="color: #edf2f7;">{{ $l->user->name ?? 'User Unit' }}</td>
                                <td class="align-middle" style="color: #cbd5e0;">{{ $l->barang->nama_barang ?? 'Barang' }}</td>
                                <td class="align-middle">{{ $l->jumlah }} Unit</td>
                                <td class="align-middle">
                                    @if($l->status == 'disetujui' || $l->status == 'ditolak')
                                        <span class="badge badge-success" style="padding: 6px 12px; border-radius: 6px;">Selesai / Dana Cair</span>
                                    @elseif($l->status == 'verifikasi')
                                        <span class="badge badge-warning" style="padding: 6px 12px; border-radius: 6px; color: #111;">Menunggu Keuangan</span>
                                    @else
                                        <span class="badge badge-secondary" style="padding: 6px 12px; border-radius: 6px;">{{ strtoupper($l->status) }}</span>
                                    @endif
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="4" class="text-center text-muted py-5" style="background-color: #111116;">
                                    <span style="font-size: 13px; color: #718096;">Belum ada riwayat aktivitas laporan keuangan.</span>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
